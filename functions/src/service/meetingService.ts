...
export const createSuddenMeeting = async (
  ...
  await Promise.all(
    teamList.docs.map(async (team) => {
      if (!!targetTeam && team.id !== targetTeam) {
        return;
      }
      const channels = await team.ref.collection("channels").get();
      channels.docs.map(async (channel) => {
        if (!!targetChannel && channel.id !== targetChannel) {
          return;
        }
        ...
        const getMemberSelectionCounts = async (
          ...
          const selectionCounts = channel.data()?.selectionCounts || {};
        ...
        const selectMembers = async (members: string[]): Promise<string[]> => {
          ...
          // Firestoreに保存
          await channel.ref.update({ selectionCounts });
        ...
        channel.data().members = channel.data().members || { members: [] };
